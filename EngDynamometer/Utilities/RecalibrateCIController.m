...
engblkmapped=[engblk '/CI Mapped Engine/Mapped CI Engine'];
trqOrFuel=get_param(engblkmapped,'InputCmdPopup');

if strcmp(engvar,'CI Mapped Engine')

   if strcmp(trqOrFuel,'Torque') %Change to CI Core if mapped engine is selected and if it is set to torque input mode
       set_param(engblk,'LabelModeActiveChoice',calengvar);
   end

elseif ~strcmp(engvar,calengvar)
    set_param(engblk,'LabelModeActiveChoice',calengvar);
end
...
if simoutexist
    ...
    ddataobjs = loaddictionaries({'CiEngineController.sldd';...
        ...
        'CiMappedEngine.sldd';...
        'SimpleEngine.sldd';...
        'SiEngine.sldd';'EngineDynamometer.sldd'});

    CiDynoSSSpdCmd=getDdData(ddataobjs{6},'CiDynoSSSpdCmd');
    CiDynoSSTrqCmd=getDdData(ddataobjs{6},'CiDynoSSTrqCmd');

    CtrlEcuCITotNBpt=getDdData(ddataobjs{1},'CtrlEcuCITotNBpt');
    CtrlEcuCITotTqBpt=getDdData(ddataobjs{1},'CtrlEcuCITotTqBpt');
    setDdData(ddataobjs{1},'CtrlEcuCITotNBpt',linspace(0,max(CiDynoSSSpdCmd(:)),length(CtrlEcuCITotNBpt)));
    setDdData(ddataobjs{1},'CtrlEcuCITotTqBpt',linspace(0,max(CiDynoSSTrqCmd(:)),length(CtrlEcuCITotTqBpt)));
    ...
    FuelMass_Table(:,1)=FuelMass_Table(:,2);  %Zero speed column cannot be properly calibrated, so copy over first non-zero speed column of fuel mass to the zero speed column.  Fuel mass must be greater than zero at zero speed to launch the vehicle when using a Mapped engine.
    ...
    if ~nomsk || doplot
        h=figure;
        set(h,'NumberTitle','off','Name',namfig);  
...
if ~doplot
    ...
    f_tbrake=GetTableData('Measured engine torque (N*m)');
    f_tbrake=f_tbrake+reshape(AccessoryTorques,size(f_tbrake));
    ...
    if strcmp(trqOrFuel,'Torque')
       ...
       f_tqmax_n_bpt=SteadyEngSpdCmdPts; 
       SteadyTrqCmdPts=linspace(0,max(f_tbrake(:)),length(SteadyEngSpdCmdPts));
    end
    ...
    f_tqmax_n_bpt(f_tqmax_n_bpt>MaxSpd)=MaxSpd;
