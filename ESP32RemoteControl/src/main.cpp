...
void setup() {
  ...
  Wire.begin(5, 6);
  if (!mpu.begin()) {
    // Keep trying to connect to MPU but don't block the serial stream
  }
...
void loop() {
  // 1. Read Inputs (LOW = Pressed because of INPUT_PULLUP)
  int accState = digitalRead(accButtonPin);
  int brkState = digitalRead(brkButtonPin);
  int honkState = digitalRead(HOOONKPin);
  int gearUpState = digitalRead(gearUpButtonPin);
  int gearDownState = digitalRead(gearDownButtonPin);
  ...
  // 4. Handle Gear Shifting (Rising Edge Detection)
  if (gearUpState == LOW && lastGearUpState == HIGH) {
    if (gear < 7) gear++;
  }
  lastGearUpState = gearUpState;

  if (gearDownState == LOW && lastGearDownState == HIGH) {
    if (gear > 1) gear--;
  }
  lastGearDownState = gearDownState;

  // 5. Gear Physics Scaling
  switch (gear) {
    case 1: minSpeed = 0.0;   maxSpeed = 50.0;  break;
    case 2: minSpeed = 30.0;  maxSpeed = 80.0;  break;
    case 3: minSpeed = 60.0;  maxSpeed = 110.0; break;
  ...
  // 6. Speed Calculation
  if (millis() - lastInputTime > 200) gasPedalPressed = false;
  ...
  if (currentSpeed < minSpeed) currentSpeed += (accSpeed * 1.5);
  currentSpeed = constrain(currentSpeed, 0.0, maxSpeed);

  // 7. DATA OUTPUT (The "CSV" format Python needs)
  // Format: accelerate,stop,honk,gear
  Serial.print(accelerate);
  Serial.print(",");
  Serial.print(stop);
  Serial.print(",");
  Serial.print(doIHonk);
  Serial.print(",");
  Serial.println(gear); // println provides the \n trigger

  delay(20); // Sync with Python's 50Hz (1000ms/20ms = 50Hz)
}